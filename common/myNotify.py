...
# 通知服务
BARK = os.environ.get('BARK_PUSH')  # bark服务,此参数如果以http或者https开头则判定为自建bark服务; secrets可填;
...
if "TG_BOT_TOKEN" in os.environ and os.environ["TG_BOT_TOKEN"] and "TG_USER_ID" in os.environ and \
        os.environ["TG_USER_ID"]:
    TG_BOT_TOKEN = os.environ["TG_BOT_TOKEN"]
    TG_USER_ID = os.environ["TG_USER_ID"]
if "DD_BOT_TOKEN" in os.environ and os.environ["DD_BOT_TOKEN"] and "DD_BOT_SECRET" in os.environ and \
...
def bark(title, content):
    ...
    if BARK.startswith('http'):
        url = f"""{BARK}/{title}/{content}?sound={bark_sound}&group={bark_group}"""
    else:
        url = f"""https://api.day.app/{BARK}/{title}/{content}?sound={bark_sound}&group={bark_group}"""
...
def serverJ(title, content):
    ...
    if response['code'] == 0:
        if response.get('errno') == 0 or response.get('data', {}).get('errno') == 0:
            print('推送成功！')
            return
    else:
        print('推送失败！{}'.format(response))
